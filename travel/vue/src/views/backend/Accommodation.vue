                ...
                <template #item="{element, index}">
                  <div class="image-item">
                    ...
                    <div class="image-overlay">
                      <el-icon class="icon-btn" @click="setMainImage(index)" v-if="!element.isMain">
                        <Star />
                      </el-icon>
                      <el-icon class="icon-btn" @click="removeImage(index)">
                        <Delete />
                      </el-icon>
                    </div>
...
const setMainImage = (index) => {
  ...
  // 更新form中的imageUrl为主图
  form.imageUrl = imageList.value[index].url
}
...
const removeImage = (index) => {
  ...
  if (removedImage.isMain && imageList.value.length > 0) {
    ...
    form.imageUrl = imageList.value[0].url
  ...
  try {
    ...
    const uploadOptions = {
      ...
      onSuccess: (data) => {
        ...
        const newImage = {
          url: data,
          isMain: imageList.value.length === 0 // 第一张图片设为主图
        }
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        const submitData = {
          ...form,
          imageList: imageList.value.length > 0 ? JSON.stringify(imageList.value) : null
        }
